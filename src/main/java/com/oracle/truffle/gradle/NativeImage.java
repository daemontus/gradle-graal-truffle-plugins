...
/**
 * <p>A task prototype for using Graal {@code native-image} tool for building native
 * binaries of Java applications. This task only works on GraalVM!</p>
 *
 * The task has several configuration options that you can use to specify the tools inputs:
 *
 * {@code
 * task compileNative(type: NativeImage) {
 *    // Set the name of the output binary (default is chosen by native-image based on main class/jar).
 *    outputName "my-awesome-binary"
 *    // A path/file inside the project where the outputs should be stored.
 *    outputDir "binaries"
 *    // Specifies the main class. Alternatively, you can use `forJarFile` or `forJarTask` to run native-image
 *    // with main jar file instead of main class.
 *    forMainClass "app.Main"
 *    // Add extra command line arguments for the native-image process.
 *    appendCmdArgs "--arg" "-x"
 *    // Add extra items to native-image classpath.
 *    appendClasspath "/my/extra.jar"
 * }
 * }
 *
 */
